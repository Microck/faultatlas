...
must_haves:
  truths:
    - "README explains what Indagine is and how to run the demo"
...
- Create `README.md` with:
  ...
  - Architecture section that explicitly clarifies what "8 agents" means for DEMO-04 so README and diagram match:
    ...
    - Note that `IndagineController` / `IndaginePipeline` are orchestration glue (shown in the diagram but not counted as an "agent").
...
- Create `docs/architecture.mmd` (Mermaid) diagram that matches the README definition of "8 agents":
  ...
  - Also show the non-agent components that wire them together: TraceStore (memory/cosmos) and IndagineController/IndaginePipeline.
