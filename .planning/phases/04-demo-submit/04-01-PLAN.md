...
must_haves:
  ...
  key_links:
    - from: "demo/run_demo.py"
      to: "src/core/indagine_pipeline.py"
      via: "live mode runs end-to-end pipeline"
      pattern: "indagine_pipeline|run_indagine"
    - from: "demo/run_demo.py"
...
- Implement `demo/run_demo.py` supporting:
  - `--mode live` to run the real, end-to-end pipeline by calling the actual modules (not `run_and_capture`):
    ...
    - Load the stored trace via `IndaginePipeline` (TraceStore injection) and run analyzers via IndagineController.
...
- Live demo exercises the actual pipeline modules end-to-end (FailureDetector -> TraceStore -> IndaginePipeline -> DiagnosisEngine -> FixGenerator)
  </done>
</task>
