# Indagine Demo Scenario (2 minutes)
...
| --- | --- | --- | --- |
| 0:00-0:15 | "AI agents fail in production, but debugging them is slow and manual. Indagine automates that investigation flow." | Show terminal at repo root and `demo/scenario.md` briefly, then clear screen. | Frame the problem and promise the flow. |
...
| 1:25-1:45 | "Now Indagine proposes a concrete fix and shows the exact diff to apply in the BookingAgent." | Scroll to `fixes[0].changes[0].diff`. | Human-reviewable fix diff, not vague advice. |
